...
- 挪车提醒 H5 页面（一键通知 / 发信息给车主 / 拨打电话）
- 企业微信应用消息推送（access_token 内存缓存 + 6s 超时控制）
...
- [ ] **一键通知乐观即时反馈**：点击后 Worker 立即返回「已通知车主」，真正的微信推送用 `event.waitUntil` 后台发送，体感从 ~2.6s 降至 ~0.4s。权衡：微信偶发失败时用户已看到成功、不会重试，车主有极小概率未收到通知。
- [ ] **频率限制升级 KV**：跨 isolate 共享计数，需新增 KV 绑定与 `wrangler.toml` 配置。
- [ ] **成功/失败 Toast 颜色区分**：当前成功与失败提示样式一致，建议成功绿色、失败红色。
